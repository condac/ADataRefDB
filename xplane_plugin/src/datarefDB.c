...
PLUGIN_API int XPluginStart(char* outName, char* outSig, char* outDesc) {
    strcpy(outName, "ADataRefDB");
    strcpy(outSig, "github.condac.ADataRefDB");
    strcpy(outDesc, "A plug-in for datarefDB.");
    ...
    //XPLMSetWindowTitle(g_window, "datarefDB");
    ...
    mySubMenuItem = XPLMAppendMenuItem(XPLMFindPluginsMenu(), /* Put in plugins menu */
                                       "ADataRefDB",          /* Item Title */
                                       0,                     /* Item Ref */
                                       1);                    /* Force English */

    /* Now create a submenu attached to our menu item. */
    myMenu = XPLMCreateMenu("ADataRefDB",
                            XPLMFindPluginsMenu(),
    ...
    XPLMDebugString("datarefDB:read config\n");
    readConfig();
...
float MyFlightLoopCallback(float inElapsedSinceLastCall, float inElapsedTimeSinceLastFlightLoop, int inCounter, void* inRefcon) {
    ...
    //XPLMDebugString("datarefDB: flightloop\n");
